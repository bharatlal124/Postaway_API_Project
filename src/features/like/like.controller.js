    ...
    if (postExists) {
      ...
      if (isLiked) {
        ...
        if (likeData) {
          return res
            .status(200)
            .send({ message: "Unliked post", unlikedData: likeData });
        } else {
          return res
            .status(500)
            .send({ error: "Something went wrong while unlike" });
        }
      } else {
        ...
        if (likeData) {
          return res
            .status(200)
            .send({ message: "Liked post", likedData: likeData });
        } else {
          return res
            .status(500)
            .send({ error: "Something went wrong while like" });
        }
      }
    } else {
